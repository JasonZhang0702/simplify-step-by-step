...
import sys
sys.path.append("/root/codes/MultiRCTS/src")
...
model_name = "/root/models/Llama-3.1-8B-Instruct"
...
DP_STARTEGY = {
    ...
    },  # policy generated by agent using dp-algorithm
}
if __name__ == '__main__':
    ...
    save_dir = "/root/codes/MultiRCTS/result/few-shot_cefrsp_28"
    os.makedirs(save_dir, exist_ok=True)

    case_dict_expert = json.load(open(f"/root/codes/MultiRCTS/data/prompt_expert/en_expert_case_cefrsp.json"))  # loading  semantic-aware exemplar selection
    case_dict_expert = padding_case(case_dict_expert)  # padding non-case

    casedf = pd.read_csv(f"/root/codes/MultiRCTS/data/en/CEFR-SP_train.csv")  # single sentence case selected in training set
    ...
    for begin_cefr in [6, 5, 4]:  # as same as Barayan et al. coling, only simpify CEFR-4, 5, 6
        df = pd.read_csv(f"/root/codes/MultiRCTS/data/en/CEFR-SP_test.csv")
        df = df[df["Rating"] == begin_cefr].reset_index(drop=True)
            
        for target_cefr in [1, 2, 3]:
            complex_sentence = df["Sentence"].tolist()
            strategy = DP_STARTEGY["en"][f"{begin_cefr}{target_cefr}"]  # getting strategy from DPagent
        ...
        df.to_csv(f"{save_dir}/en_few-shot_beginCEFR{begin_cefr}.csv", index=False, sep=',')
